...
namespace AI_assignment
{
    class DoubleQuestion
    {
        ...
        // populates the chosen attribute
        public DoubleQuestion(int attribute)
        {
        ...
        // finds the statistical varage we will use to optimize the cutting of the dataset
        public double findAvg(Dictionary<string, List<string>> dataset)
        {
            double total = 0;
            int datasetlength = 0;
            foreach (KeyValuePair<string, List<string>> entry in dataset)
            {
                try
                {
                    total += Convert.ToDouble(entry.Value[this.attribute]);
                    datasetlength++;
                }
                catch (Exception e) {
                    continue;
